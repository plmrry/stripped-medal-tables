...
function version_2(d3, $, Rx, _) { // jshint ignore:line
  ...
  return function(container, data) {
    ...
    const rows = rows_join.enter()
      ...
      .merge(rows_join)
      .attr('transform', (d) => `translate(0, ${y_cluster(d.id)})`);
...
function version_1(d3, $) { // jshint ignore:line
  return function(data) {
    ...
    frame.selectAll('.point')
      ...
      .append('circle')
      .attr('r', 3)
      .style('fill', d => ['g','s','b'].indexOf(d.medal) !== -1 ? medal_color(d.medal) : 'red');
